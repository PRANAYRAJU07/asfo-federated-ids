    ...
    def _load_kg(self):
        if os.path.exists(self.filepath):
            with open(self.filepath, 'r') as f:
                self.kg = json.load(f)
            logger.info(f"Loaded Knowledge Graph with {len(self.kg)} entries from {self.filepath}")
        else:
            logger.warning(f"Knowledge Graph file {self.filepath} not found. Using default scores.")
