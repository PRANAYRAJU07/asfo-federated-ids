...
class IDSClient(fl.client.NumPyClient):
    def __init__(self, cid: str, model: torch.nn.Module, train_loader, val_loader, 
                 optimizer: torch.optim.Optimizer, criterion, device, epochs: int, 
                 metadata: dict = None):
        self.cid = cid
        self.model = model
    ...
    def fit(self, parameters: List[np.ndarray], config: Dict[str, fl.common.Scalar]) -> Tuple[List[np.ndarray], int, Dict]:
        ...
        global_params = None
        if proximal_mu > 0.0:
            global_params = [torch.tensor(p).to(self.trainer.device) for p in parameters]
        
        self.trainer.fit(self.train_loader, self.val_loader, epochs=self.epochs,
    ...
    def evaluate(self, parameters: List[np.ndarray], config: Dict[str, fl.common.Scalar]) -> Tuple[float, int, Dict]:
        ...
        return float(loss), len(self.val_loader.dataset), {"accuracy": float(accuracy), **val_metrics}
