...
class ASFOStrategy(fl.server.strategy.FedAvg):
    ...
    def aggregate_fit(
        ...
        if self.global_parameters_cache is None:
            logger.warning(
                "Global parameters cache is empty. Using absolute weights for first round."
            )
        ...
        for _, fit_res in results:
            ...
            if self.global_parameters_cache is not None:
                delta_k = [
                    w_layer - g_layer
                    for w_layer, g_layer in zip(w_k, self.global_parameters_cache)
                ]
            else:
                delta_k = w_k
            ...
            for cls, count in dist.items():
                self.global_rarity_counts[cls] = (
                    self.global_rarity_counts.get(cls, 0) + count
                )
        ...
        if self.use_knowledge_graph:
            impact_scores = self.impact_provider.get_impact_scores(
                list(all_classes_in_round)
            )
        else:
        ...
        for cls in all_classes_in_round:
            if self.use_rarity:
                rarity_vector[cls] = 1.0 / (
                    self.epsilon + self.global_rarity_counts.get(cls, 0)
                )
            else:
                rarity_vector[cls] = 1.0
        ...
        for dist in client_distributions:
            ...
            if total_counts == 0:
                ...
                srs_k = sum(
                    [
                        f_hat.get(cls, 0)
                        * impact_scores.get(cls, 1.0)
                        * rarity_vector.get(cls, 1.0)
                        for cls in all_classes_in_round
                    ]
                )
        ...
        # Phase A: Volumetric Pseudo-Gradient (v_vol)
        p_k = [size / total_samples for size in client_sizes]
        v_vol = self._compute_weighted_sum(client_deltas, p_k)
        ...
        if self.static_lambda is not None:
            ...
            if norm_vol > 0 and norm_sem > 0:
                cos_sim = np.dot(flat_vol, flat_sem) / (
                    (norm_vol + self.epsilon) * (norm_sem + self.epsilon)
                )
        else:
        ...
        if self.global_parameters_cache is not None:
            new_global_weights = [
                g_layer + d_layer
                for g_layer, d_layer in zip(self.global_parameters_cache, final_delta)
            ]
        else:
    ...
    def _compute_weighted_sum(
        self, client_deltas: List[List[np.ndarray]], weights: List[float]
    ) -> List[np.ndarray]:
