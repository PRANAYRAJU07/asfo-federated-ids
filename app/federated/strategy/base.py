    ...
    def aggregate_fit(
        ...
        if aggregated_parameters is not None:
            self._log_and_checkpoint(
                server_round, aggregated_parameters, results, bytes_uploaded, agg_start
            )

        return aggregated_parameters, metrics_aggregated

    def _log_and_checkpoint(
        self, server_round, aggregated_parameters, results, bytes_uploaded, agg_start
    ):
        agg_end = time.time()
        ...
        logger.info(
            f"Round {server_round} | Wall time: {round_wall_time:.2f}s | Agg time: {agg_time:.2f}s | Comm: {bytes_total / 1024 / 1024:.2f} MB"
        )
    ...
    def aggregate_evaluate(
        ...
        if loss is not None and metrics:
            ...
            if acc > self.best_accuracy:
                ...
                torch.save(
                    self.global_model.state_dict(), self.checkpoint_dir / "best.pt"
                )
