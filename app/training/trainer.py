...
class Trainer:
    def __init__(self, model, optimizer, criterion, device, callbacks=None, log_dir="logs/"):
    ...
    def fit(self, train_loader, val_loader, epochs: int, proximal_mu: float = 0.0, global_params: list = None):
        ...
        for epoch in range(epochs):
            ...
            train_logs = self._train_epoch(train_loader, epoch, proximal_mu, global_params)
            ...
            stop_training = any(getattr(cb, 'stop_training', False) for cb in self.callbacks)
            if stop_training:
                break
        ...
        with torch.no_grad():
            for data, target in loader:
                ...
                if output.shape[-1] == 1:
                    ...
                    loss = self.criterion(output, target)
                    probs = torch.softmax(output, dim=-1)[:, 1] if output.shape[-1] > 1 else output
