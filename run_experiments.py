...
def run_experiment(dataset: str, strategy: str, seed: int):
    logger.info(f"Starting Experiment: Dataset={dataset}, Strategy={strategy}, Seed={seed}")
    
    cmd = [
        sys.executable,
        "-m", "uv", "run", "scripts/federated_simulation.py",
        f"datasets={dataset}",
        f"strategy={strategy}",
        f"core.seed={seed}"
    ]
    
    # We use uv run to execute within the project environment
    ...
    try:
        ...
        logger.info(f"✅ Success: {dataset} | {strategy} | Seed {seed} (Took {duration:.1f}s)")
...
def main():
    ...
    strategies = [
        ...
        "asfo_nokg", "asfo_norarity", "asfo_static", 
        "asfo_adaptive", "asfo_full"
    ]
    seeds = [42, 123, 2025, 777, 999]
    
    total_experiments = len(datasets) * len(strategies) * len(seeds)
    logger.info(f"Starting ASFO Evaluation Matrix. Total experiments: {total_experiments}")
