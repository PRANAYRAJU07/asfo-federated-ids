...
def test_model_serialization(model_name):
    ...
    # Initialize Model
    model1 = ModelRegistry.get_model(model_name, input_dim=input_dim, output_dim=output_dim)
    model1.eval()
    ...
    with TemporaryDirectory() as tmp_dir:
        ...
        # Load Model
        model2 = ModelRegistry.get_model(model_name, input_dim=input_dim, output_dim=output_dim)
        ...
        assert torch.allclose(pred1, pred2), f"Model {model_name} failed Save->Load->Predict consistency"
...
def test_model_determinism():
    ...
    assert torch.allclose(out1, out2), "Random seed did not produce identical model weights/outputs"
