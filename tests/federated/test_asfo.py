import pytest
import numpy as np
from flwr.common import FitRes, Parameters, ndarrays_to_parameters, parameters_to_ndarrays, Status, Code
...
def test_asfo_equals_fedavg_when_lambda_zero():
    ...
    results = [
        (MockClientProxy("A"), res_A),
        (MockClientProxy("B"), res_B)
    ]
...
def test_asfo_uniform_impact_rarity_dictates_weights():
    ...
    strategy = ASFOStrategy(lambda_max=0.5, temperature=1.0, use_knowledge_graph=True, use_rarity=True, impact_provider=provider)
    
    # Client 1 has 10 samples of rare class A
    # Client 2 has 100 samples of common class B
    res_1 = create_mock_fit_res([np.array([0.0])], 10, {"class_distribution": "{\"A\": 10}"})
    res_2 = create_mock_fit_res([np.array([0.0])], 100, {"class_distribution": "{\"B\": 100}"})
...
def test_asfo_uniform_rarity_impact_dictates_weights():
    ...
    strategy = ASFOStrategy(lambda_max=0.5, temperature=1.0, use_knowledge_graph=True, use_rarity=False, impact_provider=provider)
    
    res_1 = create_mock_fit_res([np.array([0.0])], 10, {"class_distribution": "{\"A\": 10}"})
    res_2 = create_mock_fit_res([np.array([0.0])], 10, {"class_distribution": "{\"B\": 10}"})
    
    strategy.aggregate_fit(1, [(MockClientProxy("1"), res_1), (MockClientProxy("2"), res_2)], [])
...
def test_asfo_no_kg_no_rarity_volumetric_behavior():
    ...
    res_1 = create_mock_fit_res([np.array([1.0])], 10, {"class_distribution": "{\"A\": 10}"})
    res_2 = create_mock_fit_res([np.array([2.0])], 10, {"class_distribution": "{\"B\": 10}"})
    
    aggregated_params, _ = strategy.aggregate_fit(1, [(MockClientProxy("1"), res_1), (MockClientProxy("2"), res_2)], [])
def test_asfo_permutation_invariance():
    ...
    res_A = create_mock_fit_res([np.array([1.0])], 10, {"class_distribution": "{\"A\": 10}"})
    res_B = create_mock_fit_res([np.array([2.0])], 20, {"class_distribution": "{\"B\": 20}"})
    res_C = create_mock_fit_res([np.array([3.0])], 30, {"class_distribution": "{\"A\": 5, \"B\": 25}"})
...
def test_asfo_equal_clients_equals_fedavg():
    ...
    res_1 = create_mock_fit_res([np.array([1.0])], 10, {"class_distribution": "{\"A\": 10}"})
    res_2 = create_mock_fit_res([np.array([3.0])], 10, {"class_distribution": "{\"A\": 10}"})
...
def test_asfo_single_client():
    ...
    res_1 = create_mock_fit_res([np.array([5.5])], 10, {"class_distribution": "{\"A\": 10}"})
